...
void uninstall_callback(CFDictionaryRef dict, int arg)
{
    int percent;
    CFStringRef status = CFDictionaryGetValue(dict, CFSTR("Status"));
    CFNumberGetValue(CFDictionaryGetValue(dict, CFSTR("PercentComplete")), kCFNumberSInt32Type, &percent);
...
CFStringRef get_bundle_id(const char *app_path)
{
    ...
    {
        printf("CFReadStreamOpen(stream) == TRUE\n");
...
void install_app(struct am_device *device)
{
    ...
    ASSERT_OR_EXIT(!AMDeviceSecureInstallApplication(0, device, local_app_url,
                                                     options, install_callback, 0), "!AMDeviceSecureInstallApplication\n");
...
void uninstall_app(struct am_device *device)
{
    ...
    ASSERT_OR_EXIT(!AMDeviceSecureUninstallApplication(0, device, bundle_id,
                                                       0, uninstall_callback, 0), "!AMDeviceSecureUninstallApplication\n");
...
void print_syntax()
{
    ...
    printf("<Commands>\n");
    printf("  -u                           : Display UDID of connected device\n");
    printf("  -b <path_to_app>             : Display bundle identifier of app (.app folder)\n");
    printf("  -i <path_to_app>             : Install app (.app folder) to device\n");
    printf("  -r <bundle_id>               : Uninstall app by bundle id\n");
    printf("  -l [-p]                      : Lists all installed apps on device\n");
    printf("  -t <from_port> <to_port>     : Forward TCP connections to connected device\n");
    printf("\n");
    printf("<Options>\n");
